...
let time_end: boolean = false;
let times_closed: number;
let { time_input_storage = 10 } = await browser.storage.local.get("time_input_storage")
let time_to_count: number = time_input_storage

const time_input = document.getElementById("time-input") as HTMLInputElement;
...
const time_checkbox = document.getElementById("time-checkbox") as HTMLInputElement;
...
const language_checkbox = document.querySelectorAll<HTMLInputElement>(".language-checkbox");
const english_checkbox = document.getElementById("english")  as HTMLInputElement;
const portuguese_checkbox = document.getElementById("portuguese")  as HTMLInputElement;
const spanish_checkbox = document.getElementById("spanish")  as HTMLInputElement;
const french_checkbox = document.getElementById("french")  as HTMLInputElement;
...
let text_1: string = ""
let top_text_left: string = ""
let top_text_right: string = ""
let bottom_text_text: string = ""

const settings_p = document.getElementById("settings-p") as HTMLInputElement;
const title = document.getElementById("title") as HTMLInputElement;
const no_wait_text = document.getElementById("no-wait-text") as HTMLInputElement;
const time_to_close_text = document.getElementById("time-to-close-text") as HTMLElement;
...
if (language_storage === "english") {
  ...
  if (english_checkbox && french_checkbox && portuguese_checkbox && spanish_checkbox && title && settings_p && no_wait_text) {
    ...
    (english_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "English";
    (french_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "French";
    (portuguese_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Portuguese";
    (spanish_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Spanish";
  ...
  if (english_checkbox && french_checkbox && portuguese_checkbox && spanish_checkbox && title && settings_p) {
    ...
    (english_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Inglês";
    (french_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Francês";
    (portuguese_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Português";
    (spanish_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Espanol";
  ...
  if (english_checkbox && french_checkbox && portuguese_checkbox && spanish_checkbox && title && settings_p) {
    ...
    (english_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Inglés";
    (french_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Francés";
    (portuguese_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Portugués";
    (spanish_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Español";
  ...
  if (english_checkbox && french_checkbox && portuguese_checkbox && spanish_checkbox && title && settings_p) {
    ...
    (english_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Anglais";
    (french_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Français";
    (portuguese_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Portugais";
    (spanish_checkbox.parentElement?.lastElementChild as HTMLElement).textContent = "Espagnol";
...
const social_media_names: Record<social_media_type, boolean> = {
...
activate_button.forEach(button => {
  ...
  const platform = button.id.replace("-button", "") as social_media_type;
...
activate_button.forEach(button => {
  button.addEventListener("click", async () => {
    ...
    const platform = button.id.replace("-button", "") as social_media_type;
    const icon = document.getElementById(`${platform}-icon`);
    social_media_names[platform] = !social_media_names[platform];
...
const font_1 = new FontFace(
  "SpaceGrotesk",
  `url(${browser.runtime.getURL("fonts/SpaceGrotesk-VariableFont_wght.ttf")})`
);

const font_2 = new FontFace(
  "Sniglet",
  `url(${browser.runtime.getURL("fonts/Sniglet-Regular.ttf")})`
);

const font_3 = new FontFace(
  "Nunito",
  `url(${browser.runtime.getURL("fonts/Nunito-Medium.ttf")})`
);
...
let img_folder: number = 0
let cat_number: number = 0
